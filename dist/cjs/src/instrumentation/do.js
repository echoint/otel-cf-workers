    ...
    const fetchHandler = {
        async apply(target, thisArg, argArray) {
            ...
            try {
                ...
                return await api_1.context.with(context, executeDOFetch, undefined, bound, request, id);
            }
            catch (error) {
                throw error;
            }
...
function instrumentAlarmFn(alarmFn, initialiser, env, id) {
    ...
    const alarmHandler = {
        async apply(target, thisArg) {
            ...
            try {
                const bound = target.bind((0, wrap_js_1.unwrap)(thisArg));
                return await api_1.context.with(context, executeDOAlarm, undefined, bound, id);
            }
            catch (error) {
                throw error;
            }
