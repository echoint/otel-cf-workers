...
const addEvent = (name, msg) => {
    ...
    api_1.trace.getActiveSpan()?.addEvent(name, attrs);
};
    ...
    const queueHandler = {
        async apply(target, _thisArg, argArray) {
            ...
            try {
                ...
                return await api_1.context.with(context, executeQueueHandler, undefined, target, args);
            }
            catch (error) {
                throw error;
            }
    ...
    const handler = {
        apply: (target, thisArg, argArray) => {
            return tracer.startActiveSpan(`Queues ${name} send`, async (span) => {
                ...
                await Reflect.apply(target, (0, wrap_js_1.unwrap)(thisArg), argArray);
                span.end();
            });
        },
    };
